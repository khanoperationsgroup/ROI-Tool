...
/**
 * Khan Operations — ROI Calculator (Highly Customizable, v2)
 * App Router (Next.js 14) + Tailwind + Recharts + html-to-image
 *
 * Notes:
 * - Create brand-new presets (blank or clone current), mark as Industry or Client
 * - Flexible KPI library per service (add/remove rows, map to effects)
 * - Colorful charts and export to PNG
 * - All math is client-side (no backend)
 */
...
type Preset = {
  id: string; name: string; category: "industry" | "client";
  baseline: Baseline; fees: Fees; kpis: { blueprint: KPI[]; odl: KPI[]; accelerator: KPI[]; }
};
...
const SCENARIO_COLOR: Record<ScenarioKey, string> = {
  Low: "#38bdf8", Base: "#22c55e", High: "#f59e0b",
};
...
export default function Page() {
  ...
  // New preset (blank/clone) form
  const [showPresetForm, setShowPresetForm] = useState(false);
  const [newPresetName, setNewPresetName] = useState("");
  const [newPresetCategory, setNewPresetCategory] = useState<"industry"|"client">("industry");
  const [newPresetMode, setNewPresetMode] = useState<"blank"|"clone">("blank");

  const createPreset = () => {
    if (!newPresetName.trim()) return;
    const id = newPresetName.toLowerCase().replace(/\s+/g, "-").replace(/[^a-z0-9-]/g, "");
    const blank: Baseline = { traffic:0, cr:0, aov:0, gm:0, subs:0, churn:0, adSpend:0, roas:0, hourly:0, wastedHrsWeek:0, errors:0, costPerError:0 };
    const newPreset: Preset = {
      id, name: newPresetName.trim(), category: newPresetCategory,
      baseline: newPresetMode === "clone" ? baseline : blank,
      fees: newPresetMode === "clone" ? fees : { blueprint:0, odl:0, accelerator:0 },
      kpis: newPresetMode === "clone" ? { blueprint:[...kpis.blueprint], odl:[...kpis.odl], accelerator:[...kpis.accelerator] } : { blueprint:[], odl:[], accelerator:[] },
    };
  ...
  const saveAsPreset = () => {
    const name = window.prompt("Preset name (e.g., 'Client — ACME Mfg'):");
    if (!name) return;
    const id = name.toLowerCase().replace(/\s+/g,"-").replace(/[^a-z0-9-]/g,"");
    const category = window.confirm("Mark this as a CLIENT preset? OK=Client, Cancel=Industry") ? "client" : "industry";
    const p: Preset = { id, name, category, baseline, fees, kpis: { blueprint:[...kpis.blueprint], odl:[...kpis.odl], accelerator:[...kpis.accelerator] } };
    setPresets((prev) => prev.some((x)=>x.id===id) ? prev.map((x)=>x.id===id?p:x) : [...prev, p]);
    setPresetId(id);
  };

  // Aggregate KPI effects for current scenario
  ...
  const scenarioData = useMemo(() => {
    return SCENARIOS.map((s) => {
      ...
      return { scenario: s.key, monthlyGP: Math.max(0, res.monthlyGP), payback: isFinite(res.payback) ? Number(res.payback.toFixed(2)) : null, roiPct: isFinite(res.roi) ? res.roi * 100 : null, color: SCENARIO_COLOR[s.key] };
    });
      ...
      <div className="bg-gradient-to-r from-indigo-600 via-fuchsia-500 to-rose-500 text-white">
        <div className="mx-auto flex max-w-6xl items-center justify-between px-4 py-6">
          ...
          <div className="flex gap-2">
            <button onClick={() => setShowPresetForm((s)=>!s)} className="rounded-xl bg-white/15 px-3 py-2 text-sm font-semibold backdrop-blur hover:bg-white/25">New Preset</button>
            <button onClick={saveAsPreset} className="rounded-xl bg-white px-3 py-2 text-sm font-semibold text-indigo-700 shadow hover:opacity-95">Save as Preset</button>
            ...
            <div className="rounded-2xl bg-white/10 p-4 backdrop-blur">
              <div className="grid grid-cols-1 gap-4 md:grid-cols-4">
                <div className="md:col-span-2">
                  ...
                  <input className="w-full rounded-xl border border-white/40 bg-white/90 px-3 py-2 text-slate-900" value={newPresetName} onChange={(e)=>setNewPresetName(e.target.value)} placeholder="e.g., Client — ACME Manufacturing"/>
                </div>
                <div>
                  <label className="mb-1 block text-sm font-medium">Category</label>
                  <select className="w-full rounded-xl border border-white/40 bg-white/90 px-3 py-2 text-slate-900" value={newPresetCategory} onChange={(e)=>setNewPresetCategory(e.target.value as any)}>
                    <option value="industry">Industry</option>
                    <option value="client">Client</option>
                  </select>
                </div>
                <div>
                  <label className="mb-1 block text-sm font-medium">Start From</label>
                  <select className="w-full rounded-xl border border-white/40 bg-white/90 px-3 py-2 text-slate-900" value={newPresetMode} onChange={(e)=>setNewPresetMode(e.target.value as any)}>
                    <option value="blank">Blank</option>
                    <option value="clone">Clone Current</option>
                  </select>
                </div>
              </div>
              <div className="mt-3 flex gap-2">
                <button onClick={createPreset} className="rounded-xl bg-white px-4 py-2 text-sm font-semibold text-indigo-700 shadow hover:opacity-95">Create Preset</button>
                <button onClick={()=>setShowPresetForm(false)} className="rounded-xl bg-white/0 px-4 py-2 text-sm font-semibold text-white hover:bg-white/10">Cancel</button>
              </div>
            </div>
          </div>
        )}
      ...
      <div className="mx-auto max-w-6xl px-4 py-6">
        <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
          ...
          <Card title="Service Fee" subtitle="One-time fee for selected service">
            ...
            {service==='odl' && <FieldNumber label="Operations Dev Lab Fee" value={fees.odl} onChange={(v)=>setFees({...fees, odl:v})}/>}
        ...
        <Card className="mt-6" refEl={exportRef} title={`Results — ${serviceTitle(service)} (${scenario} Scenario)`} subtitle="This quantifies the $-value of working with Khan Operations under your current assumptions.">
          <div className="grid grid-cols-1 gap-3 md:grid-cols-4">
            <Kpi label="Monthly GP Lift" value={fmtCurrency(result.monthlyGP)} color="bg-emerald-50 text-emerald-700"/>
            <Kpi label="Annual GP Lift" value={fmtCurrency(result.annualGP)} color="bg-sky-50 text-sky-700"/>
            <Kpi label="Payback (months)" value={isFinite(result.payback)?result.payback.toFixed(2):"—"} color="bg-violet-50 text-violet-700"/>
            <Kpi label="Annualized ROI" value={isFinite(result.roi)?`${(result.roi*100).toFixed(1)}%`:"—"} color="bg-rose-50 text-rose-700"/>
          </div>
            ...
            <div className="h-72 w-full rounded-xl border bg-white p-2">
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={scenarioData} margin={{ top: 10, right: 20, left: 0, bottom: 0 }}>
                  ...
                  <Tooltip formatter={(v:any, name:string)=> name.includes("ROI") ? f`${(v as number).toFixed(1)}%` : v } />
                  <Legend />
                  <Bar yAxisId="left" dataKey="payback" name="Payback (months)" fill="#8b5cf6" isAnimationActive>
                    <LabelList dataKey="payback" position="top" formatter={(v:number|null)=> v!=null ? v.toFixed(2) : "—"} />
                  </Bar>
                  <Bar yAxisId="right" dataKey="roiPct" name="ROI %" fill="#ef4444" isAnimationActive>
